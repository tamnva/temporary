...
!! if reservoir volume is greater than zero
     ...
     &                                 (res_evol(jres) - res_pvol(jres))
          ...
          case (3)                   !! use measured daily outflow
            ...
            resflwo = 86400. * flw
            !! code changed 
			!! Here we have the information about the water level
			!! and based upon that information the outflow was calculated using the equations defined by Prof. Luise 
          case (4)
            res_h = 0.
			resflwo = 0.
			abcdef = 0.
			ressa = 0.
			multip = 0.
			multip = res_vol(jres) *res_vol(jres)* 8* 10**(-17)
			ressa = 4256.8 + (0.000002 * res_vol(jres)) - multip
			res_h = res_vol(jres)/ressa
			res_h = (res_h*0.0001) +1335
			if (res_h> 1362.0 .and. res_h <1369.0) then
			    abcdef = res_h -1290
				resflwo = 8.44 * abcdef * abcdef* 0.001* 86400.
				else
			if (res_h> 1340.0 .and. res_h <1362.0) then
			    abcdef = (res_h -1275)**1.37
				resflwo = 9.74 * abcdef * 0.01* 86400.
				else
			if (res_h> 1293.0 .and. res_h <1340.0) then
			    abcdef = (res_h -1220)**1.684
				resflwo = 9.26 * abcdef * 0.001* 86400.
				else
				abcdef= (res_h - 1220)**1.684
				resflwo = 9.26 *abcdef* 0.001 *86400.
			end if
